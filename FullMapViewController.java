...
public class FullMapViewController
{
    ...
    public void paintPath(Map.Path path)
    {
        ...
        try {
            ...
            for (int i = 0; i < stops.length; i++) {
                ...
                System.out.println("#"+stops[i+1].name.replaceAll(". ", ""));
                System.out.println(stops[i+1].name);
                start = view.getScene().lookup("#"+stops[i].name.replaceAll(". ", ""));
                end = view.getScene().lookup("#"+stops[i+1].name.replaceAll(". ", ""));

                Line s = new Line(start.getLayoutX(), start.getLayoutY(), end.getLayoutX(), end.getLayoutY());
                s.setStroke(Paint.valueOf("Blue"));
                s.setStrokeWidth(3);
                view.getChildren().add(s);
            }
            br.close();
    ...
    void reset() // remove all painted elements
    {
        // if child is a circle or line
        // generated by intellij from foreach loop
        view.getChildren().removeIf(n -> n.getClass() == Circle.class || n.getClass() == Line.class);
    }
    @FXML
